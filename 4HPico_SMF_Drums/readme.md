...
The filesystem is set up as four banks of MIDI files to help with organization - this also helps the UI a lot on the 2HPico version of the code. On power up the code loads all of the MIDI files into RAM - not strictly needed but it makes switching files a bit faster and simplifies the menu code. When you select a file in the menu system the code parses the MIDI into a list of note on messages and event times.
...
Syncing MIDI playback to the external clock is a bit tricky. A clock interrupt measures the external clock period. The MIDI file provides a pulse per quarter note value, and the timing between note on events is derived from this PPQN value. A bit of math converts clock period, PPQN and event time into microseconds between events, which is what the note interrupt uses to schedule itself.
